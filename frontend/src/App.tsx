...
export default function App() {
  ...
  const checkAuthAndFetch = useCallback(async (showSync = false) => {
    ...
    try {
      ...
      if (!authData.configured || !authData.connected) {
  ...
  const executeUpdate = useCallback(async (updates: Partial<ACDevice>, apiCall: (id: string) => Promise<void>) => {
    ...
    setDevices((prevList) => {
      ...
      return prevList.map((d) => (d.id === activeId ? { ...d, ...updates } : d));
    });

    try {
      await apiCall(activeId);
      if (syncTimeoutRef.current) clearTimeout(syncTimeoutRef.current);
      syncTimeoutRef.current = window.setTimeout(() => checkAuthAndFetch(false), 1200);
    } catch {
      if (prevState) {
        setDevices((prevList) =>
  ...
  const handleTemp = useCallback((t: number) => {
    executeUpdate({ temperature: t }, (id) => api.setTemperature(id, t));
  }, [executeUpdate]);

  const handlePower = useCallback((p: PowerMode) => {
    executeUpdate({ power: p }, (id) => api.setPower(id, p));
  }, [executeUpdate]);

  const handleDisplay = useCallback((d: DisplayMode) => {
    executeUpdate({ display_mode: d }, (id) => api.setDisplay(id, d));
  }, [executeUpdate]);

  const handleHVAC = useCallback((m: HVACMode) => {
    executeUpdate({ hvac_mode: m }, (id) => api.setHVACMode(id, m));
  }, [executeUpdate]);

  const handleFan = useCallback((f: FanMode) => {
    executeUpdate({ fan_mode: f }, (id) => api.setFanMode(id, f));
  }, [executeUpdate]);

  const handlePreset = useCallback((p: PresetMode) => {
    executeUpdate({ preset_mode: p }, (id) => api.setPresetMode(id, p));
  }, [executeUpdate]);

  const handleConverti = useCallback((c: ConvertiMode) => {
    executeUpdate({ converti_mode: c }, (id) => api.setConvertiMode(id, c));
  }, [executeUpdate]);

  const handleVSwing = useCallback((v: SwingMode) => {
    executeUpdate({ vertical_swing_mode: v }, (id) => api.setVerticalSwing(id, v));
  }, [executeUpdate]);

  const handleHSwing = useCallback((h: SwingMode) => {
    executeUpdate({ horizontal_swing_mode: h }, (id) => api.setHorizontalSwing(id, h));
  }, [executeUpdate]);
  ...
  const handleExecutePreset = useCallback((preset: SmartMacroPreset) => {
    executeUpdate(
      {
        power: preset.power,
  ...
  if (isLoading && devices.length === 0) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-slate-400 gap-3 font-sans">
        ...
        <p className="text-xs font-medium tracking-wider uppercase">Loading MirAIe AC Hub...</p>
      </div>
    );
  }
      ...
      <div className="w-full max-w-5xl flex flex-col gap-5">
        ...
        <header className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 bg-slate-900 border border-slate-800 rounded-2xl shadow-md">
          ...
          <div className="flex items-center gap-2.5 w-full sm:w-auto justify-end">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-950 border border-slate-800 text-xs font-medium">
              <span
                className={`w-2 h-2 rounded-full ${
                  currentDevice?.online ? 'bg-emerald-400 shadow-[0_0_6px_#34d399]' : 'bg-rose-500'
                }`}
              />
            ...
            >
              <RotateCw className={`w-4 h-4 ${isSyncing ? 'animate-spin text-cyan-400' : ''}`} />
            </button>
